...
// 背景グラデーションに使用する色相をランダムに差し替えるための候補リスト
...
export function PulseGridPreview({ className = "", variant = "thumbnail" }: AnimationPreviewProps) {
  ...
  useEffect(() => {
    ...
    // gsap.context を利用してコンポーネントのアンマウント時にタイムラインを一括破棄
    const ctx = gsap.context(() => {
      // コンテナ全体の CSS カスタムプロパティを緩やかに変化させ、色相の揺らぎを演出
      gsap.to(element, {
        duration: 14,
        repeat: -1,
        ease: "none",
      ...
      // 各セルを中心から外側に向かって脈動させるステッガーアニメーション
      gsap.to("[data-cell]", {
        scale: 1.12,
        opacity: 1,
        filter: "blur(0)",
